...
exports.generateTaxSavingsSuggestions = generateTaxSavingsSuggestions;
/**
 * 2024年の税率テーブル（日本の所得税）
 * 参考: 国税庁の所得税税率
 */
 ...
 */
function calculateInhabitationTax(taxableIncome) {
    if (taxableIncome <= 0)
        return 0;
...
function calculateTax(income, expense) {
    ...
    // 総経費を計算
    const totalExpense = expense.rentExpense +
 ...
 * 節税提案を生成
 */
function generateTaxSavingsSuggestions(income, expense) {
    const suggestions = [];
    const totalExpense = expense.rentExpense +
        expense.utilityExpense +
        expense.suppliesExpense +
    ...
    if (expenseRatio < 0.3) {
        suggestions.push('経費率が低い傾向です。家賃や通信費など、事業に関連する経費を適切に計上できていないか確認してみましょう。');
    }
    if (expense.rentExpense === 0) {
        suggestions.push('自宅で仕事をされている場合は、家賃の一部を経費として計上できる可能性があります。');
    }
    if (expense.suppliesExpense === 0) {
        suggestions.push('事務用品や書籍などの消耗品費を忘れずに計上しましょう。');
    }
    return suggestions;
}
